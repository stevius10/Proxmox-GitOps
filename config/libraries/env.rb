...
module Env
  ...
  def self.set_variable(ctx, key, val, repo: nil, owner: nil, upcase: true)
    ...
    Logs.try!("set variable '#{key}' to #{val.try(:mask) || val}", [:repo, repo, :owner, owner], fail: true) do
  ...
  def self.dump(ctx, *args, repo: nil, owner: nil)
    Logs.try!("[env.dump]", [:args, args, :repo, repo, :owner, owner], fail: true) do
      node = Ctx.node(ctx); rec = nil; result = true
      get = ->(key) { Logs.try!("[env.dump] get", [:key, key], fail: false) { k = key.is_a?(String) ? key : key.to_s; return (node[key] || node[k] || node[k.to_sym]) } }
      set = ->(key, value) { return value if value.blank?; Logs.try!("[env.dump] set", [:key, key, :value, value], fail: false) { Env.set_variable(node, key, value, repo: repo, owner: owner); true } }
      req = ->(key, value) { Logs.try!("[env.dump] require", [:key, key, :value, value], fail: false) { v = value; v = v.call(node) if v.respond_to?(:call); v = node.dig(*v) if v.is_a?(Array); set.(key, v); true } }
      prc = ->(arg) do; Logs.try!("[env.dump] #{arg.inspect}", [:arg, arg], fail: false) do
        case arg
