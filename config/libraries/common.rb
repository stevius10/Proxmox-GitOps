module Common
  ...
  def packages(ctx, *pkgs, action: :install)
  ...
  def directories(ctx, dirs, owner:, group:, mode: '0755', recursive: true, action: :create)
  ...
  def download(ctx, path, url:, owner: 'root', group: 'root', mode: '0644', action: :create)
  ...
  def request(uri, user: nil, pass: nil, headers: {}, method: Net::HTTP::Get, body: nil)
  ...
  def application(ctx, name, user: 'app', group: user, exec: nil, cwd: nil, unit: {}, action: [:enable, :start], restart: 'on-failure', subscribe: nil, reload: 'systemd_reload')
    if exec || !unit.empty?
      daemon(ctx, reload)
